...
- Follow-up hardening commit `b8618e5` adds an exact nonpayable `add_product` request test and an Overview EUR page-render regression proving the sale currency is not hardcoded to GBP. It also removes accidental UTF-8 BOMs from newly added frontend files. That revision did not yet contain a page-level active-merchant/zero-state setup regression.
...
## 2026-07-29 -- Anonymous pre-deploy round 2 evidence correction -- LOCAL PASS; re-review pending

- The anonymous co-review AI reviewed exact commit `4016c21f280a37108b6bedaa91026b18bb5294d3` and returned `CHANGES REQUIRED` for one evidence-integrity blocker: the package claimed a page-level active-merchant setup regression, but `Overview.test.tsx` used a disconnected wallet and mocked `MerchantSetupActions` to `null`. The reviewer independently confirmed the two functional round-1 blockers were fixed and the full command matrix passed.
